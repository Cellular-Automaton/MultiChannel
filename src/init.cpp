...
Napi::Float64Array Wrapper::add_wrapped_multi_channel(const Napi::CallbackInfo &info)
{
    ...
    Napi::Int32Array fourth = info[3].As<Napi::Int32Array>();
    ...
    Napi::Float64Array ten = info[9].As<Napi::Float64Array>();
    Napi::Float64Array eleven = info[10].As<Napi::Float64Array>();
    Napi::Float64Array twelve = info[11].As<Napi::Float64Array>();
    Napi::Int32Array thirteen = info[12].As<Napi::Int32Array>();
    Napi::Int32Array fourteen = info[13].As<Napi::Int32Array>();
    Napi::Number fifteen = info[14].As<Napi::Number>();
    Napi::Number sixteen = info[15].As<Napi::Number>();
    Napi::Number seventeen = info[16].As<Napi::Number>();
    ...
    std::vector<double> val{Wrapper::simulate_multi_channel(
            first.Data(), second.Int32Value(), third.Int32Value(), fourth.Data(),
            fifth.Int32Value(), six.DoubleValue(), seven.DoubleValue(), eight.Data(),
            nine.Data(), ten.Data(), eleven.Data(), twelve.Data(), thirteen.Data(),
            fourteen.Data(), fifteen.DoubleValue(), sixteen.Int32Value(), seventeen.Int32Value())};
...
{
    Napi::Env env = info.Env(); // check if arguments are integer only.
    std::array<std::string, 17> tmp{"tab_init:Float64Array",    "nb_channel:Number",
